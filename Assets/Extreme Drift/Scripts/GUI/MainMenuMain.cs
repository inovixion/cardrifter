...
{
    private static MainMenuMain _instance;
    public static MainMenuMain Instance
    {
        get
        {
            if(_instance == null)
            {
                _instance = GameObject.FindObjectOfType<MainMenuMain>();
            }
            return _instance;
        }
    }
    ...
    public void ActiveCurrentColor(Image activeImage)
    {
        ...
        if (menuGUI.wheelColor.gameObject.activeSelf)
        {
            ...
            PlayerPrefsXhere.SetColor("VehicleWheelsColor" + currentVehicleNumber, mainColor);
        }
        else if (menuGUI.smokeColor.gameObject.activeSelf)
        {
            ...
            PlayerPrefsXhere.SetColor("VehicleSmokeColor" + currentVehicleNumber, new Color(mainColor.r, mainColor.g, mainColor.b, 0.2f));
        }
    ...
    public void RandomColor()
    {
        ...
        PlayerPrefsXhere.SetColor("VehicleWheelsColor" + currentVehicleNumber, vehicleSetting[currentVehicleNumber].ringMat.GetColor("_Color"));
        PlayerPrefsXhere.SetColor("VehicleSmokeColor" + currentVehicleNumber, vehicleSetting[currentVehicleNumber].smokeMat.GetColor("_TintColor"));
    }
        ...
        for (int i = 0; i < levelSetting.Length; i++)
        {
            ...
            if (levelSetting[i].bestTime)
            {
                if (PlayerPrefs.GetFloat("BestTime" + (i + 1).ToString()) != 0.0f)
                {
                    ...
                    levelSetting[i].stars.Star2.color = Color.white;
                    levelSetting[i].stars.Star3.color = Color.white;
                    levelSetting[i].bestTime.text = "BEST : " + GetComponent<FormatSecondsScripthere>().FormatSeconds(PlayerPrefs.GetFloat("BestTime" + (i + 1))).ToString();
                }
            }
        }
    ...
    void Awake()
    {
        ...
        foreach (VehicleSetting VSetting in vehicleSetting)
        {
            ...
            if (PlayerPrefsXhere.GetColor("VehicleWheelsColor" + i) == Color.clear)
            {
                vehicleSetting[i].ringMat.SetColor("_DiffuseColor", Color.white);
            }
            else
            {
                vehicleSetting[i].ringMat.SetColor("_DiffuseColor", PlayerPrefsXhere.GetColor("VehicleWheelsColor" + i));
            }



            if (PlayerPrefsXhere.GetColor("VehicleSmokeColor" + i) == Color.clear)
            {
                vehicleSetting[i].smokeMat.SetColor("_TintColor", new Color(0.8f, 0.8f, 0.8f, 0.2f));
            }
            else
            {
                vehicleSetting[i].smokeMat.SetColor("_TintColor", PlayerPrefsXhere.GetColor("VehicleSmokeColor" + i));
            }
