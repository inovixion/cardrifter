...
public class GameUIMain : MonoBehaviour
{
    ...
    public static GameUIMain manage;
    ...
    private AIVehicleDaim AIVehicleScript;
   ...
   public bool carPenalty=false;
    ...
    void Update()
    {
        ...
        panels.gamePlay.currentTime.text = "Time: " + FormatSeconds(AIControlDaim.CurrentVehicle.AIVehicle.playerCurrentTime);
        ...
        else
        {
            ...
            { 
                if (panels.gameFinish.root.activeSelf == false)
                {
                    ...
                    AIVehicleScript = AIControlDaim.CurrentVehicle.AIVehicle;
                    ...
                    else if (AIVehicleScript.playerBestTime < (panels.gameFinish.stars.Star1Time - penaltyTime))
                    {
                        ...
                        if (y == 6)
                        {
                            racePrize = (int)((Application.loadedLevel / 5.0f) * 500.0f);
                        }
                        else
                        {
                            racePrize = (int)((Application.loadedLevel / 5.0f) * 1500.0f);
    ...
    public void ShowCarUI()
    {
        ...
        gearst = AIControlDaim.CurrentVehicle.currentGear;
        carUI.speedText.text = ((int)AIControlDaim.CurrentVehicle.speed).ToString();

        if (gearst > 0 && AIControlDaim.CurrentVehicle.speed > 1)
        {
            carUI.GearText.color = Color.green;
            carUI.GearText.text = gearst.ToString();
        }
        else if (AIControlDaim.CurrentVehicle.speed > 1)
        {
        ...
        thisAngle = (AIControlDaim.CurrentVehicle.motorRPM / 20) - 175;
        thisAngle = Mathf.Clamp(thisAngle, -180, 90);

        carUI.tachometerNeedle.rectTransform.rotation = Quaternion.Euler(0, 0, -thisAngle);
        carUI.barShiftGUI.fillAmount = AIControlDaim.CurrentVehicle.powerShift / 100.0f;
