...
public class VehicleControlDaim : MonoBehaviour
{
    ...
    [HideInInspector]
    public AIVehicleDaim AIVehicle;
    ...
    void Awake()
    {
        ...
        AIVehicle = transform.GetComponent<AIVehicleDaim>();
    ...
    {
        if (collision.transform.root.GetComponent<VehicleControlDaim>())
        {
            collision.transform.root.GetComponent<VehicleControlDaim>().slip2 = Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 10.0f);
    ...
    {
        if (collision.transform.root.GetComponent<VehicleControlDaim>())
            collision.transform.root.GetComponent<VehicleControlDaim>().slip2 = 5.0f;
    ...
    void Update()
    {
        ...
        if (GameUIMain.manage.gameFinished)
            vehicleMode = VehicleMode.AICar;

        if (GameUIMain.manage.gameFailed)
            vehicleMode = VehicleMode.AICar;
        ...
        if (vehicleMode == VehicleMode.Player && GameUIMain.manage.gameStarted)
        {
            if (AIControlDaim.manage.controlMode == ControlMode.Mobile)
            {
                if (GameUIMain.manage.panels.gamePlay.buttonsUI.activeSelf)
                {
                    steer = Mathf.MoveTowards(steer, steerAmount, Time.deltaTime * (10.0f * PlayerPrefs.GetFloat("Sensitivity")));
                }
                else if (GameUIMain.manage.panels.gamePlay.accelUI.activeSelf)
                {
    ...
    void FixedUpdate()
    {
        ...
        if (vehicleMode == VehicleMode.Player && GameUIMain.manage.gameStarted)
        {
            ...
            else if (AIControlDaim.manage.controlMode == ControlMode.Mobile)
            {
        ...
        else if (vehicleMode == VehicleMode.AICar && GameUIMain.manage.gameStarted)
        {
            shift = false;
        ...
        if (carSetting.carSteer)
            carSetting.carSteer.localEulerAngles = new Vector3(steerCurAngle.x, steerCurAngle.y, steerCurAngle.z + (steer * -120.0f)* 20 * Time.deltaTime);
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (col.GetGroundHit(out hit))
            {
                ...
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    if (WGrounded)
                    {
                        GameUIMain.manage.carPenalty = true;
                        if (speed > 70) w.collider.brakeTorque = 2000;
                    }
                    else
                    {
                        GameUIMain.manage.carPenalty = false;
                    }
                    ...
                    if (WGrounded && speed > 5 && !brake)
                    {
                        ...
                        GameUIMain.manage.driftAmount = 0;
                        GameUIMain.manage.canDrift = false;
                    ...
                    else if ((brake || Mathf.Abs(hit.sidewaysSlip) > 0.2f) && speed > 1)
                    {
                        ...
                        if ((accel < 0.0f) || ((brake || Mathf.Abs(hit.sidewaysSlip) > 0.2f) && (w == wheels[2] || w == wheels[3])))
                        {
                            ...
                            if (speed > 20.0f && Mathf.Abs(hit.sidewaysSlip) > 0.1f && GameUIMain.manage.canDrift)
                            {
                                GameUIMain.manage.driftAmount += 1;
                            }
                            else
                            {
                                GameUIMain.manage.driftAmount = 0;
