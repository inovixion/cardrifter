...
public class MainMenu : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ControlFreak2.CF2Input.GetMouseButton(0) && activePanel != Panels.SelectLevel)
        {
            x = Mathf.Lerp(x, Mathf.Clamp(ControlFreak2.CF2Input.GetAxis("Mouse X"), -2, 2) * cameraRotateSpeed, Time.deltaTime * 5.0f);
        ...
        if (ControlFreak2.CF2Input.touchCount == 1&& activePanel!=Panels.SelectLevel)
        {
            switch (ControlFreak2.CF2Input.GetTouch(0).phase)
            {
                case TouchPhase.Moved:
                    x = Mathf.Lerp(x, Mathf.Clamp(ControlFreak2.CF2Input.GetTouch(0).deltaPosition.x, -2, 2) * cameraRotateSpeed, Time.deltaTime*3.0f);
                    Camera.main.fieldOfView = Mathf.Clamp(Camera.main.fieldOfView, 50, 60);
                    Camera.main.fieldOfView = Mathf.Lerp(Camera.main.fieldOfView, 50, Time.deltaTime);
                    break;
            }
