...
public class VehicleControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!carSetting.automaticGear)
        {
            if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.PageUp))
                ShiftUp();
            if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.PageDown))
                ShiftDown();
        }
    ...
    void FixedUpdate()
    {
        ...
        if (vehicleMode == VehicleMode.Player && GameUI.manage.gameStarted)
        {
            if (AIControl.manage.controlMode == ControlMode.Simple)
            {
                ...
                {
                    steer = ControlFreak2.CF2Input.GetAxis("Horizontal");
                    accel = ControlFreak2.CF2Input.GetAxis("Vertical");
                    brake = ControlFreak2.CF2Input.GetButton("Jump");
                    shift = ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) | ControlFreak2.CF2Input.GetKey(KeyCode.RightShift);
                }
